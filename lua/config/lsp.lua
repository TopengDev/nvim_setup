...
-- Filetype-based LSP setup for single files
vim.api.nvim_create_autocmd("FileType", {
  pattern = {"typescript", "javascript", "typescriptreact", "javascriptreact"},
  callback = function()
    if not is_js_ts_project() then
      -- Only start TS server for single files if not in a project
      lspconfig.ts_ls.setup({
        capabilities = capabilities,
        on_attach = on_attach,
...
vim.api.nvim_create_autocmd("FileType", {
  pattern = "solidity",
  callback = function()
    if not is_solidity_project() then
      lspconfig.solidity_ls_nomicfoundation.setup({
        capabilities = capabilities,
        on_attach = on_attach,
        single_file_support = true,
